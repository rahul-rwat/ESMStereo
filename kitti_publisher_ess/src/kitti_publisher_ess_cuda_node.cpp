...
std::string model_path_ = "/tmp/ess.plan";

nvinfer1::ICudaEngine* engine_{nullptr};
nvinfer1::IExecutionContext* context_{nullptr};
void* buffers_[4]{nullptr, nullptr, nullptr, nullptr};
cudaStream_t stream_;
int leftIndex_, rightIndex_, outputIndex_, outputConfidenceIndex_;
...
bool initializeTensorRT(const std::string plugin_path) {
    ...
    for (int i = 0; i < engine_->getNbIOTensors(); ++i) {
        ...
        for (const auto& confidenceName : confidenceNames)
            if (strcmp(name, confidenceName.c_str()) == 0) outputConfidenceIndex_ = i;
    }
    ...
    cudaMalloc(&buffers_[outputConfidenceIndex_], confidenceSize_);
...
public:
    KittiImagePublisher() : Node("kitti_image_publisher"), current_index_(0) {
        ...
        if (!initializeTensorRT(plugin_path)) {
...
private:
    void publishImages() {
        ...
        context_->setTensorAddress(engine_->getIOTensorName(outputConfidenceIndex_), buffers_[outputConfidenceIndex_]);
        ...
        cudaMemcpyAsync(confidenceOutputData, buffers_[outputConfidenceIndex_], outputSize_, cudaMemcpyDeviceToHost, stream_);
